sem.model.fits = function(modelList, aicc = FALSE) {
  ...
  if(!all(sapply(modelList, function(i) 
    ...
    all(class(i) %in% c("lm", "glm", "negbin", "gls", "pgls", "lme", "lmerMod", "merModLmerTest", "glmerMod", "glmmTMB","phylolm")) 
  ...
  ret = do.call(rbind, lapply(modelList, function(model) {
    ...
    if(any(class(model) %in% c("glm", "negbin", "gls", "pgls","phylolm"))) {
      ...
      if(any(class(model) %in% c("glm"))) ret$Marginal = (1 - (model$deviance / model$null.deviance)) else 
        ...
        if(any(class(model) %in% c("pgls","phylolm"))) ret$Marginal <- model$adj.r.squared  else {
